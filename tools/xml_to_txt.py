...
pbar = tqdm(range(len(valid_file)))
for i in pbar:
    PATH = f'{VALID_PATH}{valid_file[i]}'
    ...
    try:
        ...
        for element in root.iter('object'):
            ...
            for sub_element in element.iter('name'):
                if sub_element.text == 'face': i_class = 0
                else: i_class = 1
            for sub_element in element.iter('bndbox'):
                for index in sub_element:
                    if index.tag == 'xmin': x_min = int(index.text)
                    if index.tag == 'ymin': y_min = int(index.text)
                    if index.tag == 'xmax': x_max = int(index.text)
                    if index.tag == 'ymax': y_max = int(index.text)
                x = round((x_max/2 + x_min/2)/width, 6)
                y = round((y_max/2 + y_max/2)/height, 6)
        ...
        print("File error:", train_file[i]+'jpg')
